...
v1: 
    * Add SELECT_TF_OPS to enable IdentityN op in TFLite. 
      This will increase the interpreter binary size.
...
def restore_model(sess, ckpt_dir, enable_ema=True):
  ...
  if checkpoint == None:
    raise Exception("Can't load the latest checkpoint from ckpt_dir. Make sure ckpt_dir exists.")
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    logits, endpoints = model_builder.build_model(
        images, FLAGS.model_name, False)
    ...
    if FLAGS.quantize:
      ...
      # V1: Allow SELECT_TF_OPS in TFLite models.
  ...
  print("Supported ops: ", converter.target_spec.supported_ops)
