...
def _resize_image(image, image_size, method=None):
  if method == 'pad_and_resize':
    tf.logging.info('Use MAFF-AI custom resize method {}'.format(method))
    shapes = tf.shape(image)
    H = shapes[0]
    W = shapes[1]
    longer_side_size = tf.math.maximum(H, W)
    ## Pad and resize
    image = tf.image.resize_with_crop_or_pad(
      image, longer_side_size, longer_side_size)
...
def _decode_and_resize(image_bytes, image_size, resize_method=None):
  """Rescales image_size."""
...
def _flip_rotate(image):
  """Random horizontal/vertical image flip and rotation."""
  image = tf.image.random_flip_left_right(image)
...
def preprocess_for_train(image_bytes,
  ...
  """
  #logging.info(' - Selected image augmentation: vertical flip')
  #image = _decode_and_random_crop(image_bytes, image_size, resize_method)
  #image = _flip(image)
  logging.info(
    ' - Selected image augmentation: horizontal flip, vertical flip, rotate')
  image = _decode_and_resize(image_bytes, image_size, resize_method)
  ...
  if augment_name not in (None, "None"):
    try:
    ...
    if augment_name == 'autoaugment':
      if augment_subpolicy is None:
        raise Exception(
          'Please specify augment_subpolicy when applying autoaugment.')
      logging.info(
        'Apply AutoAugment policy %s - %s', augment_name, augment_subpolicy)
      image = autoaugment.distort_image_with_autoaugment(
        image, augment_subpolicy)
...
def preprocess_image(image_bytes,
  ...
  if is_training:
    return preprocess_for_train(
        ...
        augment_subpolicy, randaug_num_layers, randaug_magnitude, 
        resize_method)
  else:
