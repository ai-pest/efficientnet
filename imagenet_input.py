...
class ImageNetTFExampleInput(six.with_metaclass(abc.ABCMeta, object)):
  ...
  def __init__(self,
               ...
               augment_name=None,
               augment_subpolicy=None,
               mixup_alpha=0.0,
               randaug_num_layers=None,
               randaug_magnitude=None,
               resize_method=None,
               translate_classes=None):
    """Constructor.
    ...
    Args:
      ...
      augment_name: `string` that is the name of the augmentation method to
        ...
        details.
      augment_subpolicy: `string` specifying an augment subpolicy to be applied.
      ...
      translate_classes: `string` to the path to a JSON file, or `None`. If
        `string`, reads the file and translates labels accordingly.
    """
    ...
    self.augment_subpolicy = augment_subpolicy
    self.mixup_alpha = mixup_alpha
    self.randaug_num_layers = randaug_num_layers
    self.randaug_magnitude = randaug_magnitude
    self.resize_method = resize_method
    self.translate_classes = translate_classes

    ## Translate classes.
    ## JSON file should be in the following format:
    ## {
    ##   0: 0    # Noop
    ##   1: 3    # Translates index 1 to index 3
    ##   ...
    ## }
    if self.translate_classes is not None:
      with tf.io.gfile.GFile(self.translate_classes) as f:
        transl_json = json.load(f)

      self.translate_keys = []
      self.translate_values = []

      for k, v in transl_json.items():
        self.translate_keys.append(int(k)+1)    # since 0 is reserved for BG
        self.translate_values.append(v+1)

      logging.info('Translation table is initialized.') # no, not yet!
      logging.info('Keys: {}'.format(self.translate_keys))
      logging.info('Values: {}'.format(self.translate_values))
  ...
  def dataset_parser(self, value):
    ...
    image = self.image_preprocessing_fn(
        ...
        augment_subpolicy=self.augment_subpolicy,
    ...
    # Optionally translate class indices
    if self.translate_classes:
      ## Note: Moving this hashtable to __init__ raises ValueError
      ## ValueError: Tensor("hash_table:0", ...) must be from the same graph as 
      ## Tensor("input_pipeline_task0/ShuffleDataset:0", ...).
      translation = tf.lookup.StaticHashTable(
        tf.lookup.KeyValueTensorInitializer(
          tf.constant(self.translate_keys, dtype=tf.int32),
          tf.constant(self.translate_values, dtype=tf.int32)),
        default_value=-1)
  ...
  def __init__(self,
    ...
    Args:
      ...
      randaug_magnitude: 'int', if RandAug is used, what should the magnitude
        be. See autoaugment.py for detailed description.
      resize_method: If None, use bicubic in default.
      include_validation: `bool`, if True, the validation subset will also be
        used in training.
      translate_classes: `string` to the path to a JSON file, or `None`. If
        `string`, reads the file and translates labels accordingly.
    """
